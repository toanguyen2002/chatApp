...
const SendMessageGroup = () => {
    ...
    useEffect(() => {
        const getUser = async () => {
            ...
            const dataUser = await axios.post(`http://${ip}:5678/user/getUserOutCurrentGroupChat`, {
                    ...
                    },
                    {
                        ...
                        onPress: async () => {
                            try {
                                ...
                                Alert.alert("Thông báo", "Nhóm " + route.chatName + " đã được xóa")
            ...
            <SafeAreaView style={styles.modalContainer}>
                    ...
                    <View style={styles.membersContainer}>
                        ...
                        {route.users.sort((a, b) => {
                            ...
                            <TouchableOpacity
                                ...
                                style={[
                                    styles.memberButton,
                                    selectedMemberId === user._id && styles.selectedMemberButton,
                                ]}
                                onPress={() => {
                                    setSelectedMemberId(user._id);
                                }}
                            >
                                {user._id === route.groupAdmin._id && <Icon name="key" size={18} color="#FFD700" style={styles.adminIcon} />}
                ...
                <View style={styles.buttonsContainer}>
                    <View style={styles.buttonRow}>
                        ...
                        {isAdmin && <Button title="Xóa thành viên" onPress={() => handleDeleteMember(selectedMemberId)} />}
                    </View>
                    {isAdmin && (
                        <View style={styles.buttonRow}>
                    ...
                    <View style={styles.buttonRow}>
                        <Button title="Gán quyền nhóm trưởng" onPress={handleAssignGroupAdmin} />
                    </View>
                </View>
                ...
                <Modal visible={isFriendListModalVisible} animationType="slide">
                    <SafeAreaView style={styles.modalContainer}>
                        <Text style={styles.member}>Danh sách bạn bè bạn có thể thêm vào nhóm {route.chatName}</Text>
                        {/* Hiển thị danh sách bạn bè */}
                        <ScrollView>
                            {friendList.map((friend) => (
                                <TouchableOpacity style={[
                                    styles.memberButtonWaitAccept
                                ]} key={friend._id} onPress={() => handleSelectFriend(friend._id)}>
                                    <Text>{friend.name}</Text>
                                </TouchableOpacity>
                            ))}
                        </ScrollView>
                        <Button title="Đóng" onPress={closeFriendListModal} />
                    </SafeAreaView>
                </Modal>
                {/* Modal sửa đổi tên nhóm */}
                <Modal visible={isRenameModalVisible} animationType="slide">
                    <SafeAreaView style={styles.modalContainer}>
                        <Text style={styles.chatName}>Sửa đổi tên nhóm</Text>
                        <TextInput
                            style={styles.input}
                            ...
                            autoCorrect={false}
                        />
                        <Button title="Xác nhận" onPress={handleRenameGroup} />
                        <Button title="Hủy" onPress={closeRenameModal} />
                    </SafeAreaView>
                </Modal>
            </SafeAreaView>
        </Modal>
